...
- `PCBs/` – KiCad projects + production Gerbers, in two supported revisions (see **Which board version?** below):
  - `PCBs/v2.0/` (MRF-Pro-v8) – adds a dedicated low-noise LiDAR regulator on the breakout, and reworks the main board's FPC power delivery and switch: v1 feeds the breakout one switched 3.3 V rail on FPC pins 1 and 6; v2 gives the LDO its own `VBAT` feed on pin 1 and adds full deterministic shutdown.
...
Two board revisions are supported, and **the latest firmware runs on both, unchanged** — there are no board-specific builds, pin maps, or config flags. Connectors, sensors, displays, optics, printed parts, and the main board's component placement and outline are common to both, **but the main board's LiDAR power delivery was significantly reworked, alongside the breakout hardware.** On v1 the breakout runs entirely off one rail: the Feather's 3.3 V leaves the main board through the J4 power switch and comes back on FPC pins 1 and 6, so the LiDAR's laser supply, its logic supply and the STEMMA QT bus all hang off the same undecoupled net. That is the laser-pulse brownout this respin fixes. v2 splits the rails: pin 1 carries `VBAT` straight to the breakout's own LDO, and pin 6 stays the switched `3.3OUT` that gates it. Choose by the LiDAR range you need:
...
| FPC power delivery (main board) | One rail for everything: switched Feather 3.3 V on **both** pins 1 and 6 | Two rails: pin 1 carries **`VBAT`** to the breakout's LDO, pin 6 carries the gated, switched **`3.3OUT`** that enables it |
